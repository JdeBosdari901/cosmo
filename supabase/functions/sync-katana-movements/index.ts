// sync-katana-movements — v3 (EdgeRuntime.waitUntil + ef_run_log)
//
...
//
// v3 architecture: the request handler returns immediately with
// {status: "accepted", run_id, ...}. The actual work runs in the background
// via EdgeRuntime.waitUntil — Pro-plan wall-clock budget is 400s, vs the
// 150s synchronous response timeout. This is the canonical Supabase pattern
// for long-running EFs.
//
// Run observability:
//   - workflow_health row (success/error) — for EF Watchdog
//   - ef_run_log row (full stats + decisions + log + errors) — for debugging
//   - ef_state bookmark advance — only on clean, non-dry-run completion
//
...
//   Tables: ef_state, ef_run_log, katana_stock_sync, katana_products,
...
// EdgeRuntime is a Supabase global; declare for type-checker satisfaction
...
async function runEverything(runId: string, dryRun: boolean, startedAt: string): Promise<void> {
  ...
  try {
    ...
    if (movements.length === 0) {
      exitReason = "no_movements";
      log.push("Exit: no_movements");
    } else {
      ...
      // ── 6. Advance bookmark (+1ms; only on clean non-dry-run) ────────────
    ...
    // ── 7. ef_run_log insert ─────────────────────────────────────────────
    try {
      await supabase.from("ef_run_log").insert({
        run_id: runId,
        ef_slug: EF_SLUG,
        dry_run: dryRun,
      ...
      console.error("ef_run_log insert failed:", e instanceof Error ? e.message : String(e));
    }

    // ── 8. workflow_health upsert ────────────────────────────────────────
    try {
      const healthRow = errors.length === 0
        ? { last_success_at: completedAt }
        : { last_error_at: completedAt, last_error_message: errors.slice(0, 3).join(" | ") };
      ...
      console.error("workflow_health upsert failed:", e instanceof Error ? e.message : String(e));
    }
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  // Fire the long-running task in the background; do NOT await.
  EdgeRuntime.waitUntil(runEverything(runId, dryRun, startedAt));
